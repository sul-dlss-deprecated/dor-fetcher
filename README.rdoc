...
This gem provides a basic wrapper for reaching the API of an instance of dor-fetcher-service: https://github.com/sul-dlss/dor-fetcher-service.  This ReadMe assumes your familiar with the RESTFUL API of dor-fetcher-service, its JSON return and are just looking to leverage that with this gem.  
...
* Initialize via df = DorFetcher::Client.new({:service_url => 'http://YOUR_URL_HERE})
** By default this will point to http://127.0.0.1:3000, so df = DorFetcher::Client.new will initialize with localhost as the url of the service.
...
This can be passed anywhere you see params listed as an argument.  Past them in as a hash ({YOUR_STUFF}).  You do not need to include an empty hash if you have no parameters.  You can do df.get_count_for_apo(apo_druid) or df.get)_count_for_apo(apo, {:count_only=>true}) 
...
The above return a JSON string of all members of the collection, unless you pass in :count_only => true as a param.  If you're doing that though, why not use the df.get_count_for_* methods
