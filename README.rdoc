...
By default you will only get fully accessioned objects in your result set.  There is a parmeter you can specify to return all objects, but you cannot restrict these by date.
...
* :status=>'registered'   This will return ALL objects, even those that are not accessioned.  If you leave this off, you will ONLY get accessioned objects.  If this parameter is set, you cannot use the first_modified and last_modified parameters, they will be ignored since they only apply to published dates.
...
The above returns a hash of all members of the collection, unless you pass in :count_only => true as a param.  If you're doing that though, why not use the df.get_count_for_OBJECT methods?
...
Wrap any of the calls above in df.druid_array to produce just an array of druids.  This array will contain the druid prefix.  Example call:
...
* Release to RubyGems via the command rake release

== Command Line Usage

If you have the gem code checked out, you can execute gem commands on the command line by running "bin/console".  This gives you a Ruby console with the gem loaded:
